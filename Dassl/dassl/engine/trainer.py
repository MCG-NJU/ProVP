...
class SimpleTrainer(TrainerBase):
    ...
    def after_epoch(self):
        ...
        if (self.epoch + 1) % self.cfg.TRAIN.PRINT_FREQ == 0:
            result = self.test()
    ...
    def tne_test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            image_feat = output[1]
            output = output[0]
            image_feats.append(image_feat.cpu().numpy())
            print(image_feat.shape)
            labels.append(label.cpu().numpy())
            self.evaluator.process(output, label)
        print("Save image features as: ", self.output_dir)
        image_feats = np.array(image_feats,dtype=object)
        labels = np.array(labels,dtype=object)
        np.save(self.output_dir+"/img_feats.npy", image_feats)
        np.save(self.output_dir + "/labels", labels)
        results = self.evaluator.evaluate()
        for k, v in results.items():
            tag = "{}/{}".format(split, k)
            self.write_scalar(tag, v, self.epoch)
